...
$pr9 = getPravoohranDataByScore($conn, 'ПР9');
createSheetByScore($spreadsheet, 'ПР9 по баллам', $pr9);

// Получаем данные для ПР11 (после 11 класса)
$pr11 = getPravoohranDataByScore($conn, 'ПР11');
createSheetByScore($spreadsheet, 'ПР11 по баллам', $pr11);

// Отправляем файл пользователю
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment;filename="Списки_по_баллам.xlsx"');
...
function getPravoohranDataByScore($conn, $type) {
    $sql = "
        SELECT 
            abit.id_abit, 
...
function createSheetByScore($spreadsheet, $sheetName, $data) {
