...
while ($spec = $resultAllSpecs->fetch_assoc()) {
    ...
    // Заголовки таблицы
    // ... (предыдущий код остается без изменений до части с заголовками)

    // Заголовки таблицы
    $headers = [
        '№', '№ абитуриента', 'ФИО', 'Телефон', 'Социальный статус',
        'Образовательное учреждение', 'Год окончания', 'Средний балл',
        'Аттестат/диплом', 'Нужда в общежитии', 'Отметка о выдаче документов', 
        'Примечание', 'Оригиналы документов', 'Паспортные данные (серия/номер, дата выдачи, кем выдан)', 'СНИЛС'
    ];
    ...
    $sql = "SELECT abit.*, zayav.original 
            FROM abit 
            JOIN zayav ON abit.id_abit = zayav.id_abitur 
            WHERE zayav.id_spec_prof = ? 
            ORDER BY 
                CASE 
                    WHEN (LOWER(abit.pervooch_priem) LIKE '%сво%' OR 
                        LOWER(abit.pervooch_priem) LIKE '%СВО%' OR 
                        LOWER(abit.pervooch_priem) LIKE '%Сво%' OR 
                        LOWER(abit.pervooch_priem) LIKE '%ветер%' OR 
                        LOWER(abit.pervooch_priem) LIKE '%ветеран%' OR 
                        LOWER(abit.pervooch_priem) LIKE '%боевых действий%') AND zayav.original = 1 THEN 1
                    WHEN zayav.original = 1 THEN 2
                    ELSE 3
                END,
    ...
    while ($row = $result->fetch_assoc()) {
        // Формируем полные паспортные данные
        $passportData = [
            $row['ser_num'] ?? '',
            !empty($row['date_vidachi_doc']) ? date('d.m.Y', strtotime($row['date_vidachi_doc'])) : '',
            $row['kem_vidan_doc'] ?? ''
        ];
        $fullPassportData = implode(', ', array_filter($passportData));
        ...
        $data = [
            ...
            $row['original'] ? 'Оригинал' : '',
            $fullPassportData, // Полные паспортные данные
            $row['snils'] ?? '' // СНИЛС
        ];
        ...
        // Проверка на СВО/ветеранов
        $isSvoOrVeteran = preg_match('/сво|СВО|Сво|ветер|ветеран|боевых действий/i', $row['pervooch_priem']);
                        
        // Подсветка строк с оригиналами
        if ($row['original']) {
            if ($isSvoOrVeteran) {
                // Оранжевая подсветка для СВО/ветеранов с оригиналами
                $sheet->getStyle("A$rowNum:O$rowNum")->getFill()
                    ->setFillType(Fill::FILL_SOLID)
                    ->getStartColor()->setARGB('FFFFA500');
            } else {
                // Зеленая подсветка для остальных оригиналов
                $sheet->getStyle("A$rowNum:O$rowNum")->getFill()
                    ->setFillType(Fill::FILL_SOLID)
                    ->getStartColor()->setARGB('FFA0FFA0');
            }
        }
    ...
    $sheet->getColumnDimension('N')->setWidth(30); // Увеличили ширину для паспортных данных
...
header('Content-Disposition: attachment; filename="Абитуриенты_с_оригиналами_' . date('Y-m-d') . '.xlsx"');
