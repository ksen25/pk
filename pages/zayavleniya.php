...
if (!empty($abitIds)) {
    ...
    while ($row = $zayavResult->fetch_assoc()) {
        ...
        $abitData[$idAbit]['zayavleniya'][] = [
            ...
            'comment' => $row['comment'],
            'issue_note' => isset($row['issue_note']) ? $row['issue_note'] : null    
        ];
...
foreach ($abitData as $idAbit => $data) {
    ...
    $isIssued = !empty($firstZayav['issue_note']) && strpos($firstZayav['issue_note'], 'Документы выданы') === 0;
    ...
    foreach ($data['zayavleniya'] as $zayav) {
        $isIssuedRow = !empty($zayav['issue_note']) && strpos($zayav['issue_note'], 'Документы выданы') === 0;
